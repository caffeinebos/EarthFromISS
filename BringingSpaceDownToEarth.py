...
import random, os, requests, time #webbrowser
...
def unique(Missnum):
    ...
    list_set = set(Missnum)
    # convert the set to the list
    unique_list = (list(missionset))
    # find the length of the list
    listlen = (len(unique_list))
    # find the number of the second last list item 
    listlenmin1 = int(listlen-1)
    # insert "and"
    unique_list.insert(listlenmin1,"and")
    # concatenate the list into a single list
    joinedlist = [', '.join(unique_list)]
    # convert it to a string
    Finalprint = str(joinedlist)
    # clean up the string
    rmb = Finalprint.strip("[]")
    rmq = rmb.strip("'")
    rmc = rmq.rstrip(",")
    rpac = rmc.replace("and,","and")
    print (rpac + "!")
...
for URL in range(1,20):
    #Generate a random mission number between 10 and 69
    mission = random.randint(10,69)
...
missionset = set(Missnum)
 
 
#Print the number of photos downloaded this session, as well as the total number of photos
print("Wow, those are some cool photos! \nWe found and downloaded a total of " + str(len(PictureCounter)) + " pictures of the Earth! You got photos from these expeditions!")
unique(Missnum)
print("You've now downloaded a total of " + str(len(fileList)) + " unique photos!")
time.sleep(35)
